...
program agree, byable(recall) sortpreserve rclass
	...
	*if ("`pbgraph'"!="" & "`pbgraph'"!="reg" & "`pbgraph'"!="ci" & "`pbgraph'"!="both") print_error "pbgraph() invalid"
	...
	quietly {
		...
		* passing_bablok descriptive statistics
	...
	* descriptive statistics
	...
	foreach v of varlist `y' `x' {
		...
		qui su `v' if `touse'
		local m`v' = r(mean)			// Lin
		local v`v' = r(Var)				// Lin
		di as res %5.0f `r(N)' " " %5.0f (`ncases' - `r(N)') " " %5.0f `ncases' " " _c
		if (`show'==0 | `show'==2) {
			di as res %9.0g `r(mean)' "  " %9.0g `r(sd)' "  " _c
			qui ci `v' if `touse', level(`level')
			di as res %9.0g `r(lb)' " " %9.0g `r(ub)' _c
		}
		di ""
	}
