*! version 0.0.1  18apr2018 - _ttest.ado original code v1.3.3 (13jan2017) modified by R Sesma
program define _ttestx, rclass
	version 6.0
/*
...
program define ByNumber, rclass
	...
	***check for reverse option
	local rev = strpos("`options'","reverse")
	local options = subinstr("`options'","reverse","",.)
	...
	quietly {
		...
		if !`rev' {
			...
			*** if reverse, use min value as reference
			scalar `val1' = r(max)
			scalar `val2' = r(min)
		}
